...
const lMeshes = ['L001', 'L002', 'L003', 'L004', 'L005', 'L006', 'L007', 'L008',
                 ...
                 'L105', 'L106', 'L107', 'L108', 'L110', 'L109', 'L111', 'L112',
...
function setupUploadHandlers() {
  ...
  items.forEach(item => {
    ...
    input.addEventListener('change', (e) => {
      ...
      handleFile(file, preview, section, clearBtn, errorEl, dragText, item.key);
    });
...
function handleFile(file, preview, section, clearBtn, errorEl, dragText, key) {
  if (!file) return;
  ...
  document.getElementById("downloadBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        a.href = url;
        a.download = `kitchen_design_${currentDesignIndex + 1 || 1}.webm`;
        a.click();
